#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_gemm_sigmoid
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : Gemm(x, W, b) -> Sigmoid
...
nodes = [
    oh.make_node("Gemm",    ["X","W","b"], ["gemm"], transB=1),
    oh.make_node("Sigmoid", ["gemm"],      ["Y"]),
]
graph = oh.make_graph(nodes, "gemm_sig",
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO gemm_sigmoid (max_abs={max_abs:.4f})")
    sys.exit(0)
