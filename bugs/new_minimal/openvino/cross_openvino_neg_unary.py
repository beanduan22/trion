#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_neg_unary
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : Conv(3x3) -> Neg
Root cause : OV CPU Conv fp32 tiling differs from ORT; negation exposes sign diff.
Tolerance  : 0.01
...
nodes = [
    ...
    oh.make_node("Neg",  ["conv"],  ["Y"]),
]
graph = oh.make_graph(nodes, "neg_unary",
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO neg_unary (max_abs={max_abs:.4f})")
    sys.exit(0)
