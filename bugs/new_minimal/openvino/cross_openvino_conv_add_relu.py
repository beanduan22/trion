#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_conv_add_relu
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : Conv(3x3) -> Add(bias) -> Relu
Root cause : OV CPU Conv fp32 inner-dim sum tiling differs from ORT.
Tolerance  : 0.01
...
nodes = [
    oh.make_node("Conv", ["X","W","b"], ["conv"], kernel_shape=[3,3], pads=[1,1,1,1]),
    oh.make_node("Relu", ["conv"],      ["Y"]),
]
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO conv_add_relu (max_abs={max_abs:.4f})")
    sys.exit(0)
