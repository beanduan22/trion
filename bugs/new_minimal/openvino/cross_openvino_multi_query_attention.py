#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_multi_query_attention
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : Multi-query attention: Q from large MatMul, shared K/V -> attention
Root cause : OV CPU GEMM fp32 accumulation differs from ORT in Q projection.
Tolerance  : 0.01
...
# K, V are single-head (shared across query heads)
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO multi_query_attention (max_abs={max_abs:.4f})")
    sys.exit(0)
