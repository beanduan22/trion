#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_where_mask_fill
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : MatMul(x,W) -> Where(mask, result, fill_val)
Root cause : OV CPU GEMM fp32 accumulation differs from ORT; Where exposes.
Tolerance  : 0.01
...
mask = (np.random.rand(2, 64) > 0.5).astype(bool)
fill = np.full((2, 64), -1e9, dtype=np.float32)

nodes = [
    oh.make_node("MatMul", ["X","W"],         ["mm"]),
    oh.make_node("Where",  ["mask","mm","fill"], ["Y"]),
]
graph = oh.make_graph(nodes, "where_fill",
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO where_mask_fill (max_abs={max_abs:.4f})")
    sys.exit(0)
