#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on reciprocal_mul
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

Reciprocal → Reshape → MatMul: torch.compile crashes on onnx2torch Reshape with runtime shape tensor
...
np.random.seed(42)
x = np.random.uniform(0.01, 0.1, (4, 16)).astype(np.float32)
W = np.random.randn(16, 8).astype(np.float32) * 10
s = np.array([4, 16], dtype=np.int64)

nodes = [
    oh.make_node("Reciprocal", ["X"],   ["recip"]),
    oh.make_node("Reshape",    ["recip","s"], ["flat"]),
    oh.make_node("MatMul",     ["flat","W"], ["Y"]),
]
graph = oh.make_graph(nodes, "recip_reshape",
    [oh.make_tensor_value_info("X", TP.FLOAT, [4, 16])],
    [oh.make_tensor_value_info("Y", TP.FLOAT, [4, 8])],
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on reciprocal_mul")
