#!/usr/bin/env python3
"""
Bug ID     : cross_torch_compile_gemm_sigmoid
Compiler   : torch.compile (Inductor backend), torch 2.9.x
Oracle     : ORT_DISABLE_ALL and eager onnx2torch — both run correctly
Patterns   : Gemm (Linear) -> Sigmoid -> Reshape(runtime_shape).
Root cause : Gemm + Sigmoid converts cleanly eager/ORT.  The trailing
...
wg = (np.random.randn(OUT, K) * 0.1).astype(np.float32)
bg = np.zeros(OUT, dtype=np.float32)
s = np.array([B, OUT], dtype=np.int64)  # runtime shape triggers compile crash

nodes = [
    oh.make_node("Gemm", ["X", "W", "B"], ["g"], alpha=1.0, beta=1.0, transB=1),
    oh.make_node("Sigmoid", ["g"], ["sig"]),
    oh.make_node("Reshape", ["sig", "s"], ["Y"]),
]
graph = oh.make_graph(
    nodes,
    "gemm_sigmoid",
...
print(f"ORT: shape={ref.shape}  range=({ref.min():.3f},{ref.max():.3f}) first4={ref.ravel()[:4]}")
