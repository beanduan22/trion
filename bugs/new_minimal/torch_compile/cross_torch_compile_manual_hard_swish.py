#!/usr/bin/env python3
"""
Bug ID     : cross_torch_compile_manual_hard_swish
Compiler   : torch.compile (Inductor backend), torch 2.9.x
Oracle     : ORT_DISABLE_ALL and eager onnx2torch — both run correctly
Patterns   : Manual hard-swish decomposition: y = x * relu6(x + 3) / 6
             expressed as Add -> Clip(0,6) -> Mul -> Div.  Then
             Reshape(runtime_shape).
Root cause : The decomposed hard-swish lowers cleanly eager/ORT.  The
...
np.random.seed(42)
B, C = 4, 32
x = (np.random.randn(B, C) * 4.0).astype(np.float32)
...
s = np.array([B, C], dtype=np.int64)

nodes = [
    oh.make_node("Add", ["X", "three"], ["xp3"]),
    oh.make_node("Clip", ["xp3", "lo", "hi"], ["relu6"]),
    oh.make_node("Mul", ["X", "relu6"], ["x_mul"]),
    oh.make_node("Div", ["x_mul", "six"], ["hswish"]),
    oh.make_node("Reshape", ["hswish", "s"], ["Y"]),
]
graph = oh.make_graph(
    nodes,
    "manual_hard_swish",
    [oh.make_tensor_value_info("X", TP.FLOAT, [B, C])],
    [oh.make_tensor_value_info("Y", TP.FLOAT, [B, C])],
    initializer=[
        onh.from_array(three, "three"),
        onh.from_array(six, "six"),
