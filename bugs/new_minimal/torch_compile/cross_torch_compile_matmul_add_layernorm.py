#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on matmul_add_layernorm
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

matmul_add_layernorm pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "matmul_add_layernorm_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on matmul_add_layernorm")
