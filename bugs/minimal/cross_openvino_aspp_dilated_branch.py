#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_aspp_dilated_branch
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : Conv(3x3,dil=2) + Conv(3x3,dil=4) branches -> Add
Root cause : OV CPU Conv with large C_IN fp32 tiling differs from ORT.
Tolerance  : 0.01
...
W1 = np.random.randn(64, 32, 3, 3).astype(np.float32)
W2 = np.random.randn(64, 32, 3, 3).astype(np.float32)

nodes = [
    oh.make_node("Conv", ["X","W1"], ["c1"], kernel_shape=[3,3], dilations=[2,2], pads=[2,2,2,2]),
    oh.make_node("Conv", ["X","W2"], ["c2"], kernel_shape=[3,3], dilations=[4,4], pads=[4,4,4,4]),
    oh.make_node("Add",  ["c1","c2"], ["Y"]),
]
graph = oh.make_graph(nodes, "aspp",
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO aspp_dilated_branch (max_abs={max_abs:.4f})")
    sys.exit(0)
