#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_glu
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : MatMul(x, W) -> Split -> Sigmoid(gate) -> Mul  [GLU]
...
W  = np.random.randn(512, 128).astype(np.float32)  # project to double width
sp = np.array([64, 64], dtype=np.int64)

nodes = [
    oh.make_node("MatMul", ["X","W"],      ["proj"]),
    oh.make_node("Split",  ["proj","sp"],  ["val","gate"], axis=1),
    oh.make_node("Sigmoid",["gate"],       ["gate_s"]),
    oh.make_node("Mul",    ["val","gate_s"], ["Y"]),
]
graph = oh.make_graph(nodes, "glu",
    [oh.make_tensor_value_info("X", TP.FLOAT, [2,512])],
    [oh.make_tensor_value_info("Y", TP.FLOAT, [2,64])],
    initializer=[onh.from_array(W,"W"), onh.from_array(sp,"sp")])
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO glu (max_abs={max_abs:.4f})")
    sys.exit(0)
