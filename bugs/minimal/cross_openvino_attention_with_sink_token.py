#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_attention_with_sink_token
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : MatMul(Q,Kt) -> Add(sink_bias) -> Softmax -> MatMul(xV)
Root cause : OV CPU GEMM fp32 accumulation differs; sink token bias amplifies.
Tolerance  : 0.01
...
# sink token: first token always attended to (large positive bias)
sink  = np.array([[5.0, 0.0], [5.0, 0.0]], dtype=np.float32)
...
nodes = [
    ...
    oh.make_node("Add",      ["scores_s","sink"], ["biased"]),
    oh.make_node("Softmax",  ["biased"],      ["attn"], axis=-1),
    oh.make_node("MatMul",   ["attn","V"],    ["Y"]),
]
graph = oh.make_graph(nodes, "sink_attn",
    ...
    initializer=[
        ...
        onh.from_array(scale,"sc"), onh.from_array(sink,"sink"),
    ])
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO attention_with_sink_token (max_abs={max_abs:.4f})")
    sys.exit(0)
