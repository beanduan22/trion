#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on slice_full_range
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

slice_full_range pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "slice_full_range_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on slice_full_range")
