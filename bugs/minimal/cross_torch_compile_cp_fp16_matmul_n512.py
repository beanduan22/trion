#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on cp_fp16_matmul_n512
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

cp_fp16_matmul_n512 pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "cp_fp16_matmul_n512_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on cp_fp16_matmul_n512")
