#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on triple_add_residual
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

triple_add_residual pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "triple_add_residual_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on triple_add_residual")
