#!/usr/bin/env python3
"""
Bug ID     : tf_61865 / compiler: TorchScript (torch.jit.trace) via onnx2torch
Source     : https://github.com/tensorflow/tensorflow/issues/61865
Compiler   : TorchScript (torch.jit.trace) after onnx2torch conversion
Patterns   : Gather(params[8], indices=[2,5,4,7,8,3]) — index 8 is OOB for size-8 tensor
Root cause : TorchScript inherits PyTorch's strict bounds-checking, raising IndexError
             at runtime for any OOB access.  OpenVINO returns 0.0 silently; TF-XLA
             clamps silently.
             Compiler comparison:
               ...
               TorchScript       → IndexError      ← TARGET
               TF-XLA (original) → [1,1,1,1,1,1]  (silent clamp)
Exit 0 = BUG REPRODUCED (TorchScript errors while other compilers return values)
...
# ── Build ONNX model ─────────────────────────────────────────────────────────
# New input: params size=4, all different; index 4 is OOB for size-4
params  = np.array([3., 1., 4., 1.], dtype=np.float32)    # first digits of pi
indices = np.array([0, 1, 2, 3, 4], dtype=np.int64)       # index 4 is OOB

nodes = [oh.make_node("Gather", ["params", "indices"], ["Y"], axis=0)]
inits = [onh.from_array(indices, "indices")]
g = oh.make_graph(nodes, "gather_oob",
    [oh.make_tensor_value_info("params", TP.FLOAT, [4])],
    [oh.make_tensor_value_info("Y",      TP.FLOAT, [5])],
    initializer=inits)
...
print(f"indices : {indices.tolist()}  (index 4 is OOB for size-4 tensor)")
print()
...
for name, (out, err) in results.items():
    ...
    flag    = "  ← TARGET" if name == "TorchScript" else ""
    print(f"{name:<16}  {out_str:<32}  {err_str}{flag}")

# ── Verdict ───────────────────────────────────────────────────────────────────
ts_out, ts_err   = results.get("TorchScript", (None, "not run"))
others_with_value = {k: results[k][0].tolist() for k in results
                     if k != "TorchScript" and results[k][0] is not None}

print()
if ts_err and others_with_value:
    print(f"BUG REPRODUCED: TorchScript raises on OOB Gather")
    print(f"  while other compilers return values: {others_with_value}")
    sys.exit(0)
elif not ts_err:
    print("NOT REPRODUCED: TorchScript returned a value.")
    sys.exit(1)
else:
