#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on ia_sat_sub_uint8_underflow
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

ia_sat_sub_uint8_underflow pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
s = np.array([2, 64], dtype=np.int64)   # runtime shape → FakeTensor in compile

nodes = [
    oh.make_node("MatMul",  ["X", "W"], ["mm"]),
    oh.make_node("Reshape", ["mm", "s"], ["Y"]),
]
graph = oh.make_graph(nodes, "ia_sat_sub_uint8_underflow_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on ia_sat_sub_uint8_underflow")
