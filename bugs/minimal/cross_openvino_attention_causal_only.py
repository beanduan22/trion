#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_attention_causal_only
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : MatMul(Q,Kt) -> Add(causal_mask) -> Softmax -> MatMul(xV)
Root cause : OV CPU GEMM fp32 accumulation differs; causal mask + Softmax amplifies.
Tolerance  : 0.01
...
mask = np.triu(np.full((2,2), -1e9, dtype=np.float32), k=1)
...
nodes = [
    ...
    oh.make_node("Add",      ["scores_s","mask"], ["masked"]),
    oh.make_node("Softmax",  ["masked"],      ["attn"], axis=-1),
    oh.make_node("MatMul",   ["attn","V"],    ["Y"]),
]
graph = oh.make_graph(nodes, "causal_attn",
    ...
    initializer=[
        ...
        onh.from_array(scale,"sc"), onh.from_array(mask,"mask"),
    ])
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO attention_causal_only (max_abs={max_abs:.4f})")
    sys.exit(0)
