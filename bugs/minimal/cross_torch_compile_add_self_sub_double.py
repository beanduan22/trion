#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on add_self_sub_double
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

add_self_sub_double pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "add_self_sub_double_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on add_self_sub_double")
