#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on conv_manual_celu
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

conv_manual_celu pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "conv_manual_celu_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on conv_manual_celu")
