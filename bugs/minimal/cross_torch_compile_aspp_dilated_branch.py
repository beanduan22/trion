#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on aspp_dilated_branch
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

aspp_dilated_branch pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "aspp_dilated_branch_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on aspp_dilated_branch")
