#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on flatten_gemm
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

flatten_gemm pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "flatten_gemm_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on flatten_gemm")
