#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on multi_query_attention
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

multi_query_attention pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "multi_query_attention_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on multi_query_attention")
