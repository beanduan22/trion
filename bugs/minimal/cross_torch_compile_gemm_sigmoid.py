#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on gemm_sigmoid
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

gemm_sigmoid pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "gemm_sigmoid_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on gemm_sigmoid")
