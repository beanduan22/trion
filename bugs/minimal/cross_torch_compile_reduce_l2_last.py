#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on reduce_l2_last
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

reduce_l2_last pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "reduce_l2_last_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on reduce_l2_last")
