#!/usr/bin/env python3
"""
Bug ID     : jax_xla_log_abs_eps
Source     : Campaign v1 (fuzzing)
Compiler   : JAX/XLA (jax.jit)
Patterns   : log_abs_eps
Root cause : XLA JIT fast-math or reassociation changes log(abs+eps) precision
Tolerance  : 0.01
...
def f(x):
    return jnp.log(jnp.abs(x) + 1e-5) * x
