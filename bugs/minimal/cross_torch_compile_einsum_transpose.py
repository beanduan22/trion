#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on einsum_transpose
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

einsum_transpose pattern: MatMul → Reshape; torch.compile fails on onnx2torch Reshape with runtime shape.
...
graph = oh.make_graph(nodes, "einsum_transpose_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on einsum_transpose")
