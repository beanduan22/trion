#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_add_self_sub_double
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : MatMul(x, W) -> Add(R) -> Add(R) -> Sub(R) -- double add/sub residual
...
nodes = [
    ...
    oh.make_node("Add",    ["mm", "b"],   ["add1"]),
    oh.make_node("Add",    ["add1", "R"], ["add2"]),
    oh.make_node("Sub",    ["add2", "R"], ["Y"]),
]
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO add_self_sub_double (max_abs={max_abs:.4f})")
    sys.exit(0)
