#!/usr/bin/env python3
"""
Bug ID     : cross_openvino_transpose_matmul_transpose
Compiler   : OpenVINO CPU
Oracle     : ORT_DISABLE_ALL
Patterns   : Transpose -> MatMul -> Transpose
Root cause : OV CPU GEMM fp32 accumulation differs after transpose reorder.
Tolerance  : 0.01
...
np.random.seed(42)
x  = np.random.randn(512, 2).astype(np.float32)   # transposed input
W  = np.random.randn(512, 64).astype(np.float32)

nodes = [
    oh.make_node("Transpose", ["X"],     ["Xt"], perm=[1,0]),   # [2, 512]
    oh.make_node("MatMul",    ["Xt","W"], ["mm"]),               # [2, 64]
    oh.make_node("Transpose", ["mm"],    ["Y"],  perm=[1,0]),   # [64, 2]
]
...
if max_abs > 0.01:
    print(f"BUG REPRODUCED: OpenVINO transpose_matmul_transpose (max_abs={max_abs:.4f})")
    sys.exit(0)
