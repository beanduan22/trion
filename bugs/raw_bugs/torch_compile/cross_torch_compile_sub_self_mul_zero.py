#!/usr/bin/env python3
"""
Bug: torch.compile (Inductor) crashes on sub_self_mul_zero
Compiler: torch.compile
Oracle:   ORT_DISABLE_ALL — runs correctly; eager onnx2torch also runs correctly

Sub(x, x) → Reshape → MatMul: torch.compile crashes on onnx2torch Reshape with runtime shape tensor
...
s    = np.array([2, 64], dtype=np.int64)   # runtime shape tensor

nodes = [
    oh.make_node("Sub",    ["X", "X"],  ["zero"]),
    oh.make_node("Reshape",["zero","s"],["flat"]),
    oh.make_node("MatMul", ["flat","W"],["Y"]),
]
graph = oh.make_graph(nodes, "sub_self_reshape",
...
try:
    ...
    print(f"BUG REPRODUCED: torch.compile crashes on sub_self_mul_zero")
