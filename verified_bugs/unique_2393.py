#!/usr/bin/env python3
"""
Bug #2393: jax.jit (XLA) produces wrong output vs PyTorch eager reference.
...
Run         : python unique_2393.py  →  exit 0 = BUG REPRODUCED
"""
import sys
import numpy as np
...
_w5 = np.frombuffer(bytes.fromhex("0000000000000000"), dtype=np.int64).reshape([1])
...
_w8 = np.frombuffer(bytes.fromhex("0100000000000000"), dtype=np.int64).reshape([1])
