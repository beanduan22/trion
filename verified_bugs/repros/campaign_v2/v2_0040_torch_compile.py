#!/usr/bin/env python3
"""
Bug v2-0040 — torch.compile wrong output vs pytorch_eager.
...
Root cause: [SUSPECT: all 6 compilers diverge] torch.compile may miscompile gather-reshape + matmul+bias+GELU — GELU approximation uses wrong tanh argument in the fused kernel.
...
Run: python v2_0040_torch_compile.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (torch.compile) at idx 48: {result.ravel()[48]:.6f}")  # expected=-0.001027, got=0.000000
