#!/usr/bin/env python3
"""
Bug v2-0000 — OnnxRuntime wrong output vs ONNX spec reference.
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT gives numerically incorrect output for ONNX Resize(coordinate_transformation_mode=asymmetric, mode=linear) combined with int32 cast roundtrip — the double_transpose + resize interaction likely triggers incorrect buffer reuse in the graph optimizer.
...
The ONNX model is embedded in unique_2619.py (the reference repro).
Run: python v2_0000_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
...
if not os.path.exists(_ref):
    print("Reference unique_2619.py not found")
    sys.exit(1)
