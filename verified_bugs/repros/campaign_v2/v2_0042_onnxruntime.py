#!/usr/bin/env python3
"""
Bug v2-0042 — OnnxRuntime wrong output vs ONNX spec reference.
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT ReduceSum + mul-zero elimination: optimizer eliminates x*0 before ReduceSum, but the zero multiplication was needed to broadcast shapes — the shape mismatch silently produces wrong sum.
...
The ONNX model is embedded in unique_2661.py (the reference repro).
Run: python v2_0042_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
...
if not os.path.exists(_ref):
    print("Reference unique_2661.py not found")
    sys.exit(1)
