#!/usr/bin/env python3
"""
Bug v2-0009 — OnnxRuntime wrong output vs ONNX spec reference.
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT Conv+HardSwish fusion gives numerically different results than unfused sequence — fused kernel uses clip(x/6+0.5, 0, 1) with different rounding boundary than reference.
...
The ONNX model is embedded in unique_2628.py (the reference repro).
Run: python v2_0009_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
...
if not os.path.exists(_ref):
    print("Reference unique_2628.py not found")
    sys.exit(1)
