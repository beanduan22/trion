#!/usr/bin/env python3
"""
Bug v2-0034 — OpenVINO CPU wrong output vs pytorch_eager reference.
...
Root cause: [SUSPECT: all 6 compilers diverge] OV may incorrectly eliminate self-sub-zero (x-x→0) under TopK + instance norm — NaN-unsafe optimization may differ from pytorch_eager.
...
Run: python v2_0034_openvino.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (OpenVINO CPU) at idx 0: {result.ravel()[0]:.6f}")  # expected=-0.000045, got=0.000000
