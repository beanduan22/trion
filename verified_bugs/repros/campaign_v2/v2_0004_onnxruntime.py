#!/usr/bin/env python3
"""
Bug v2-0004 — OnnxRuntime wrong output vs ONNX spec reference.
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT Resize nearest_ceil gives wrong pixel coordinates when combined with Conv+BN fusion — the fused BN scale/shift alters values before resize causing wrong nearest-ceil rounding.
...
The ONNX model is embedded in unique_2623.py (the reference repro).
Run: python v2_0004_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
...
if not os.path.exists(_ref):
    print("Reference unique_2623.py not found")
    sys.exit(1)
