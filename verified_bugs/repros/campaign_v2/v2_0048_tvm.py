#!/usr/bin/env python3
"""
Bug v2-0048 — Apache TVM wrong output vs pytorch_eager reference.
...
Root cause: [SUSPECT: all 6 compilers diverge] TVM relay may miscompile matmul+bias+GELU under double layernorm — GELU tanh approximation uses wrong argument in TE schedule.
...
Run: python v2_0048_tvm.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Expected (pytorch_eager) at idx 43: {ref.ravel()[43]:.6f}")
    print(f"Got (TVM relay) at idx 43: {result.ravel()[43]:.6f}")  # expected=0.255722, got=0.268611
