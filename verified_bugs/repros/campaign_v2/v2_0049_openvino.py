#!/usr/bin/env python3
"""
Bug v2-0049 — OpenVINO CPU wrong output vs pytorch_eager reference.
...
Root cause: OpenVINO CPU incorrectly eliminates mul-by-zero under matmul+bias+GELU with gated linear branch — OV arithmetic optimizer removes a near-zero multiplication with non-trivial accumulated effect.
...
Run: python v2_0049_openvino.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (OpenVINO CPU) at idx 0: {result.ravel()[0]:.6f}")  # expected=0.000000, got=-0.000000
