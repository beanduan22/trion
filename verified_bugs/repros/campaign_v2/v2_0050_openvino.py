#!/usr/bin/env python3
"""
Bug v2-0050 — OpenVINO CPU wrong output vs pytorch_eager reference.
...
Root cause: [SUSPECT: all 6 compilers diverge] OV miscomputes cumsum on last axis combined with int32 cast roundtrip — CumSum plugin may have off-by-one in last-axis prefix sum for this cast pattern.
...
Run: python v2_0050_openvino.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os, io
import numpy as np
...
def main():
    ...
    print(f"Got (OpenVINO CPU) at idx 0: {result.ravel()[0]:.6f}")  # expected=1.545121, got=2.627582
