#!/usr/bin/env python3
"""
Bug v2-0029 — OnnxRuntime wrong output vs ONNX spec reference.
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT Resize nearest_ceil gives wrong coordinates when combined with 4D batched matmul — optimizer reorders matmul before resize changing the scale factor used for coordinate mapping.
...
The ONNX model is embedded in unique_2648.py (the reference repro).
Run: python v2_0029_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
...
if not os.path.exists(_ref):
    print("Reference unique_2648.py not found")
    sys.exit(1)
