#!/usr/bin/env python3
"""
Bug v2-0014 — Apache TVM wrong output vs pytorch_eager reference.
...
Root cause: TVM relay miscompiles cumsum on last axis combined with GEMM-tanh and GELU-gated branch — TVM cumsum TE generates wrong schedule for last-axis accumulation in this topology.
...
Run: python v2_0014_tvm.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (TVM relay) at idx 97: {result.ravel()[97]:.6f}")  # expected=3.184480, got=-0.277236
