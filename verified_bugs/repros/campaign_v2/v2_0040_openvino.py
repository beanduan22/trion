#!/usr/bin/env python3
"""
Bug v2-0040 — OpenVINO CPU wrong output vs pytorch_eager reference.
...
Root cause: [SUSPECT: all 6 compilers diverge] OV may miscompute matmul+bias+GELU combined with gather-reshape and manual layernorm — GELU or gather implementation diverges from pytorch_eager.
...
Run: python v2_0040_openvino.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (OpenVINO CPU) at idx 48: {result.ravel()[48]:.6f}")  # expected=-0.001027, got=0.000000
