#!/usr/bin/env python3
"""
Bug v2-0048 — OpenVINO CPU wrong output vs pytorch_eager reference.
...
Root cause: [SUSPECT: all 6 compilers diverge] OV may miscompute matmul+bias+GELU under double layernorm — GELU approximation or sqrt-reciprocal-mul interaction diverges from pytorch_eager.
...
Run: python v2_0048_openvino.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Expected (pytorch_eager) at idx 58: {ref.ravel()[58]:.6f}")
    print(f"Got (OpenVINO CPU) at idx 58: {result.ravel()[58]:.6f}")  # expected=0.268121, got=0.254915
