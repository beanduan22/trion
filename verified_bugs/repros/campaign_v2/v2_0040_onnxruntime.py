#!/usr/bin/env python3
"""
Bug v2-0040 — OnnxRuntime wrong output vs pytorch_eager reference.
...
Root cause: [SUSPECT: all 6 compilers diverge] ORT may miscompute matmul+bias+GELU combined with gather-reshape, reduce_l2_last, and manual layernorm — GELU approximation difference or gather implementation issue.
...
Run: python v2_0040_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (OnnxRuntime) at idx 48: {result.ravel()[48]:.6f}")  # expected=-0.001027, got=0.000000
