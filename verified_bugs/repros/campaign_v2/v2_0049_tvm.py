#!/usr/bin/env python3
"""
Bug v2-0049 — Apache TVM wrong output vs pytorch_eager reference.
...
Root cause: TVM relay incorrectly eliminates mul-by-zero under matmul+GELU with gated linear branch — TVM constant folding removes a zero-multiply branch that is not globally dead.
...
Run: python v2_0049_tvm.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (TVM relay) at idx 0: {result.ravel()[0]:.6f}")  # expected=0.000000, got=0.000001
