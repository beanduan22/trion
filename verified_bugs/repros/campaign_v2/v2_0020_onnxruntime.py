#!/usr/bin/env python3
"""
Bug v2-0020 — OnnxRuntime wrong output vs ONNX spec reference.
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT Resize(nearest, asymmetric) gives wrong pixel mapping when combined with ada-layernorm residual — optimizer fuses the layernorm into resize changing coordinate offsets.
...
The ONNX model is embedded in unique_2639.py (the reference repro).
Run: python v2_0020_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
...
if not os.path.exists(_ref):
    print("Reference unique_2639.py not found")
    sys.exit(1)
