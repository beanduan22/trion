#!/usr/bin/env python3
"""
Bug v2-0050 — torch.compile wrong output vs pytorch_eager.
...
Root cause: [SUSPECT: all 6 compilers diverge] torch.compile may miscompile cumsum combined with int32 cast roundtrip — Inductor memory layout pass may not handle cast-roundtrip correctly.
...
Run: python v2_0050_torch_compile.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os, io
import numpy as np
...
def main():
    ...
    print(f"Got (torch.compile) at idx 0: {result.ravel()[0]:.6f}")  # expected=1.545121, got=1.544717
