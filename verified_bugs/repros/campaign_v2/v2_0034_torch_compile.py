#!/usr/bin/env python3
"""
Bug v2-0034 — torch.compile wrong output vs pytorch_eager.
...
Root cause: [SUSPECT: all 6 compilers diverge] torch.compile may incorrectly eliminate self-sub-zero (x-x→0) — NaN-unsafe algebraic simplification.
...
Run: python v2_0034_torch_compile.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (torch.compile) at idx 0: {result.ravel()[0]:.6f}")  # expected=-0.000045, got=0.000000
