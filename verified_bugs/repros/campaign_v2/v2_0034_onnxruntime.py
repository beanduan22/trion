#!/usr/bin/env python3
"""
Bug v2-0034 — OnnxRuntime wrong output vs pytorch_eager reference.
...
Root cause: [SUSPECT: all 6 compilers diverge] ORT may incorrectly eliminate self-sub-zero (x-x→0) under temperature layer norm + TopK — NaN propagation could cause legitimate difference from pytorch_eager.
...
Run: python v2_0034_onnxruntime.py  →  exit 0 = BUG REPRODUCED
"""
import sys, os
import numpy as np
...
def main():
    ...
    print(f"Got (OnnxRuntime) at idx 0: {result.ravel()[0]:.6f}")  # expected=-0.000045, got=0.000122
