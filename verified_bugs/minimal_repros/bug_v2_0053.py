#!/usr/bin/env python3
"""
Bug v2-0053 — ORT: Resize(nearest, half_pixel, ceil) + abs(x)*x before resize + dilated ASPP
Compiler : OnnxRuntime
Pattern  : Abs(x)*x -> Resize(nearest,half_pixel,ceil) -> 4D MatMul -> dilated ASPP conv
Root cause: ORT's nearest-ceil resize with half_pixel coordinate mode may compute
            wrong pixel coordinates. The abs(x)*x pre-processing (signed square)
            interacts with the ceil rounding in ways ORT fuses incorrectly.

Run: python bug_v2_0053.py
"""
import numpy as np
import onnx
