#!/usr/bin/env python3
"""
Bug v2-0047 — ORT: L2-norm (x^2 -> ReduceSum -> Sqrt) + mul+add chain gives wrong result
Compiler : OnnxRuntime
Pattern  : x^2 -> ReduceSum -> Add(eps) -> Sqrt -> Div  (L2 norm) then Div+Mul+Add chain
Root cause: ORT may fuse the L2-norm pattern into an LpNormalization op, then the
            subsequent Div/Mul/Add chain on the normalized result gives a different
            numerical result than the unfused path.

Run: python bug_v2_0047.py
"""
import numpy as np
import onnx
