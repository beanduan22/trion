#!/usr/bin/env python3
"""
Bug v2-0055 — ORT: Resize(nearest_ceil) + manual layernorm + dilated ASPP + ConstantOfShape
Compiler : OnnxRuntime
Pattern  : 4D MatMul -> manual LayerNorm -> dilated ASPP -> ConstantOfShape -> Resize(nearest,half_pixel,ceil)
Root cause: ORT nearest-ceil resize with half_pixel coordinate mode after dilated ASPP
            convolutions may compute wrong coordinate mapping. ConstantOfShape adding
            zero-bias before resize may trigger incorrect constant folding.

Run: python bug_v2_0055.py
"""
import numpy as np
import onnx
