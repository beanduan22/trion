#!/usr/bin/env python3
"""
Bug v2-0018 — ORT CumSum(last axis) + transpose-matmul-transpose + ALiBi attention
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT optimizer fuses the double Transpose-MatMul-Transpose pattern
            (a no-op reshape of the matmul) but applies it incorrectly around
            the CumSum output, producing wrong results when the CumSum feeds
            into ALiBi-biased attention.
"""
import numpy as np
import onnx
