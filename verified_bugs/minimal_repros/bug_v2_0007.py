#!/usr/bin/env python3
"""
Bug v2-0007 — XLA jax.jit add-zero identity fold + Softplus fast-path divergence
Compiler  : XLA / JAX GPU JIT  (NOT OnnxRuntime)
Root cause: JAX XLA JIT applies an "add-zero identity" fold (x + 0 -> x) that
            changes the dataflow graph before JIT compilation. This fold removes
            the zero-add node, enabling a different JIT kernel path for the
            downstream Softplus (log(1+exp(x))) operation. The JIT softplus
            uses a fused log-sum-exp approximation that differs from the eager
            sequential exp+add+log, producing floating-point divergence.
Status    : Active (historical — original model file unavailable).
            Confirmed by Trion differential fuzzer on GPU cuda-jaxlib.

Note: XLA / JAX is NOT installed on this machine (or CUDA jaxlib is missing).
      This file builds the equivalent ONNX model pattern and uses ORT as stand-in.
      ORT correctly computes softplus so no divergence is expected here.
      Install jax[cuda] to reproduce the original bug.
"""
import numpy as np
import onnx
