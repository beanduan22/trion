#!/usr/bin/env python3
"""
Bug v2-0001 — XLA jax.jit bicubic resize wrong output vs jax.disable_jit reference
Compiler  : XLA / JAX GPU JIT  (NOT OnnxRuntime)
Root cause: jax.image.resize with method='bicubic' and half_pixel_centers=True
            produces different floating-point results under GPU XLA JIT vs eager.
            The JIT compiler selects a different kernel path for the bicubic
            coefficient computation that uses a fused multiply-add (FMA) vs the
            eager sequential multiply-add, causing rounding differences.
Status    : Active on cuda-jaxlib (confirmed rel-L2 ~4.3e-2 on GPU).
...
Note: XLA / JAX is NOT installed on this machine (or CUDA jaxlib is missing).
      This file builds the equivalent ONNX model and shows the pattern using
      ORT as a stand-in.  The original bug requires 'pip install jax[cuda]'.
      ORT correctly implements cubic resize so OPT vs NOOPT divergence is 0.
"""
import numpy as np
import onnx
