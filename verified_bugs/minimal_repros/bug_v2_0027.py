"""
Bug v2-0027 — ORT: Resize(linear, asymmetric) after residual double-conv with shared weights
Compiler  : OnnxRuntime
Root cause: ORT incorrectly fuses a residual double-Conv block (both branches sharing the same
            weight and bias tensors) with the subsequent Resize(linear, asymmetric, 2x) upsampler.
            The shared-weight aliasing confuses the graph optimizer, which replaces the linear
            resize with a wrong resampling kernel.
Pattern   : Input -> Conv(w1,b) -> Relu -> Conv(w1,b) -> Add(input) ->   [residual]
            -> Resize(linear, asymmetric, 2x)
            -> MM4D -> Sigmoid -> Mul (swish)
            -> Slice -> Slice -> Concat
"""
import numpy as np
import onnx
