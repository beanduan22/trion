"""
Bug v2-0028 — ORT: Resize(nearest_ceil, half_pixel) + ReduceSum(middle axis) combo
Compiler  : OnnxRuntime
Root cause: ORT incorrectly fuses Resize(nearest_ceil) with a downstream ReduceSum that
            reduces over the spatial-height axis (axis=2).  The optimizer reassociates
            the reduction with the upsampled output incorrectly, producing wrong results
            when ReduceSum output is broadcast-added back to the full upsampled tensor.
Pattern   : Input(1,3,32,32) -> Resize(nearest_ceil, half_pixel, 2x) ->
            ReduceSum(axis=2, keepdims=0) -> Add(resize_out) ->
            sqrt(x^2 + eps) * scale -> Relu -> Add -> Relu -> InstanceNorm -> Relu
"""
import numpy as np
import onnx
