#!/usr/bin/env python3
"""
Bug v2-0005 — ORT TopK(k=1) tie-breaking diverges from pytorch after x-x=0 const folding
...
Root cause: When ORT const-folds Sub(X, X) -> all-zero tensor, the resulting zero
            tensor is passed to TopK(k=1, largest=True).  For all-equal inputs, the
            tie-breaking index is implementation-defined.  ORT always returns index=0,
            while pytorch torch.topk returns a higher index (often the last one or
            an arbitrary stable index depending on the backend).
            The downstream LayerNorm on the constant output compounds the effect.
Status    : Active (ORT_DISABLE_ALL == ORT_ENABLE_ALL; divergence is vs pytorch_eager)

Minimal trigger: Sub(X, X) -> TopK(k=1, axis=-1, largest=True) on zeros
  ORT index=0, pytorch index=D-1 (or other index) for identical all-zero input.
"""
import numpy as np
import onnx
