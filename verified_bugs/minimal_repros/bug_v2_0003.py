#!/usr/bin/env python3
"""
Bug v2-0003 — XLA jax.jit bicubic resize + BatchNorm eval fusion divergence
Compiler  : XLA / JAX GPU JIT  (NOT OnnxRuntime)
Root cause: JAX XLA JIT fuses bicubic resize with BatchNorm scale/shift into a
            single XLA kernel. The fused kernel reorders floating-point operations
            (affine scaling inside the interpolation loop) relative to the eager
            sequential approach, causing rounding divergence on GPU.
Status    : Active (historical — original model file unavailable).
            Confirmed by Trion differential fuzzer. XLA GPU required to reproduce.

Note: XLA / JAX is NOT installed on this machine (or CUDA jaxlib is missing).
      This file builds the equivalent ONNX model and uses ORT as a stand-in.
      The XLA JIT divergence is NOT observable through ORT.
