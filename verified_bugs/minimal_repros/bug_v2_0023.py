"""
Bug v2-0023 — TF XLA: dual-path Conv-BN + shared-weight Conv + Resize(linear, align_corners)
...
Root cause: XLA GPU JIT fuses dual conv paths (one through BN, one through scale+bias) that share
            the same weight tensor.  The fused result is then fed through an identity-reshape
            round-trip before Resize(linear, align_corners), which triggers wrong coordinate mapping
            in XLA's aligned linear upsampler.
Pattern   : MM4D -> [Conv-BN path] + [Conv-scale-bias path] -> Add ->
            Reshape(flat)->Reshape(back)->Reshape(flat)->Reshape(back) ->
            Resize(linear, align_corners) -> Concat(zeros)
"""
import numpy as np
import onnx
