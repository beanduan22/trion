"""
Bug v2-0021 — TF XLA: DepthToSpace + Resize(linear, align_corners) + double Transpose wrong output
...
Root cause: JAX/XLA GPU JIT miscompiles a pipeline that pixel-shuffles via DepthToSpace (CRD mode),
            then applies linear resize with align_corners coordinate mapping, followed by
            a double NHWC<->NCHW transpose round-trip.  ORT shows the same divergence between
            optimised and reference execution because ORT's align_corners + fused-transpose path
            contains a similar edge case.
Pattern   : DepthToSpace(CRD, block=2) -> Conv1x1 -> Resize(linear, align_corners) ->
            Transpose(NCHW->NHWC) -> Transpose(NHWC->NCHW) -> Where(>0) -> Mul
"""
import numpy as np
import onnx
