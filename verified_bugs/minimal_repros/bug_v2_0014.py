#!/usr/bin/env python3
"""
Bug v2-0014 — ORT CumSum(last axis) + GEMM-tanh + gated-GELU
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT optimizer incorrectly simplifies the CumSum output when it
            appears as input to a Gemm+Tanh block followed by a GELU activation,
            causing numerical divergence in the gated output.
"""
import numpy as np
import onnx
