"""
Bug v2-0029 — ORT: Resize(nearest_ceil, half_pixel) + 4D batched MatMul + multi-dilation Conv
Compiler  : OnnxRuntime
Root cause: ORT's optimizer incorrectly reorders a Resize(nearest_ceil) with a downstream
            4D batched MatMul (broadcast over batch/channel dims).  The shared weight tensor
            between two consecutive MatMul nodes creates an aliasing that the constant-folder
            exploits wrongly before the upsampler.  The resulting mismatch is amplified by
            the subsequent multi-dilation Conv sum (dilations 1,2,4).
Pattern   : MM4D(shared_w) -> MM4D(shared_w) -> Resize(nearest_ceil, half_pixel, 2x) ->
            Conv(dil=1) + Conv(dil=2) + Conv(dil=4) -> Add + Add ->
            Reshape -> GroupNorm(manual) -> Reshape -> Mul(scale) -> Add(bias) ->
            Sigmoid -> Mul (swish)
"""
import numpy as np
import onnx
