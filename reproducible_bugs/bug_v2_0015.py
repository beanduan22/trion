#!/usr/bin/env python3
"""
Bug v2-0015 — ORT Resize(nearest_ceil) + softsign activation before resize
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT optimizer fuses the Softsign -> Mul -> Where gating pattern
            before the nearest_ceil Resize, producing incorrect upsampled output
            when the gating mask is folded into a constant incorrectly.
"""
import numpy as np
import onnx
