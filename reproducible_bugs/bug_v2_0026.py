"""
Bug v2-0026 — ORT: MatMul+scale+bias + Cast(fp32->fp16->fp32) roundtrip + mul-zero-elim
Compiler  : OnnxRuntime
Root cause: ORT's optimizer incorrectly eliminates the multiply-by-zero (mul-zero-elim pass)
            after a Cast fp32->fp16->fp32 roundtrip, because it treats the result as exactly
            zero and propagates that through LayerNorm+residual, defeating the subsequent
            channel-attention (SENet) gate.  The cast-roundtrip introduces rounding noise
            that should survive the zero-mul but is erroneously folded.
Pattern   : MM(256->256) -> scale -> bias -> Cast(fp16) -> Cast(fp32) ->
            Mul(zeros) -> Add(scale) ->
            LayerNorm -> Add(residual) -> ReduceMean ->
            MM->bias->Relu->MM->bias->Sigmoid -> Mul -> LayerNorm
"""
import numpy as np
import onnx
