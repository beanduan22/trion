#!/usr/bin/env python3
"""
Bug v2-0016 — ORT Resize(linear, asymmetric) + unsqueeze+squeeze chain
Compiler  : OnnxRuntime (ORT_ENABLE_ALL)
Root cause: ORT optimizer eliminates the Unsqueeze/Squeeze identity chain after
            asymmetric linear Resize, but the shape assumptions it makes about
            the intermediate tensor are incorrect, causing incorrect ELU-variant
            activation results downstream.
"""
import numpy as np
import onnx
