#!/usr/bin/env python3
"""
Bug v2-0008 — XLA jax.jit bicubic resize + dilated conv branch + edge-mode pad divergence
...
Root cause: JAX XLA JIT fuses a dilated convolution branch with an edge-mode padding
            operation (jnp.pad with mode='edge'). The XLA kernel for edge-padding
            inside a fused dilated-conv computes boundary values using a different
            floating-point order than the sequential eager path, causing divergence
            at the spatial boundaries of the feature map.
            Combined with bicubic resize (same half_pixel JIT issue as v2-0001),
...
Note: XLA / JAX is NOT installed on this machine (or CUDA jaxlib is missing).
      ...
      ORT does not exhibit this bug. Install jax[cuda] to reproduce the original.
"""
import numpy as np
import onnx
