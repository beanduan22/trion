#!/usr/bin/env python3
"""
bug_0151 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.4334
Input shape    : [1, 3, 32, 32]
Patterns       : broadcast/where_mask -> branch/dual_pool_add -> branch/spatial_attention_cbam -> fusion/conv_transpose_bn_relu -> branch/multi_scale_conv_branch -> constant/learned_temperature_scale

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1628'}

Usage:
    python reproduce/bug_0151.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0151.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1628)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1628)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1628)")


if __name__ == "__main__":
    print("Reproducing bug_0151 [DISCREPANCY]")
    print("  patterns : broadcast/where_mask -> branch/dual_pool_add -> branch/spatial_attention_cbam -> fusion/conv_transpose_bn_relu -> branch/multi_scale_conv_branch -> constant/learned_temperature_scale")
