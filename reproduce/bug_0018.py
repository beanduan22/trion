#!/usr/bin/env python3
"""
bug_0018 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : tflite, torch_compile
Total score    : 7.0115
Input shape    : [1, 64, 64]
Patterns       : normalization/ada_layer_norm -> attention/gated_mlp_block -> constant/cast_roundtrip -> normalization/manual_layernorm -> broadcast/where_mask -> constant/identity_chain

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '1.0000', 'tflite': '1.0000'}

Usage:
    python reproduce/bug_0018.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0018.onnx")
...
def test_discrepancy_tflite():
    ...
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 64, 64]).astype(np.float32)}
    try:
...
if __name__ == "__main__":
    print("Reproducing bug_0018 [DISCREPANCY]")
    print("  patterns : normalization/ada_layer_norm -> attention/gated_mlp_block -> constant/cast_roundtrip -> normalization/manual_layernorm -> broadcast/where_mask -> constant/identity_chain")
    print("  input    : [1, 64, 64]")
