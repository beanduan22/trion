#!/usr/bin/env python3
"""
bug_0072 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.5880
Input shape    : [1, 16, 128]
Patterns       : broadcast/euclidean_norm_broadcast -> constant/self_sub_zero -> constant/cast_roundtrip -> normalization/layernorm_residual_add -> attention/gated_mlp_block -> branch/residual_add_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2938'}

Usage:
    python reproduce/bug_0072.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0072.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2938)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2938)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2938)")


if __name__ == "__main__":
    print("Reproducing bug_0072 [DISCREPANCY]")
    print("  patterns : broadcast/euclidean_norm_broadcast -> constant/self_sub_zero -> constant/cast_roundtrip -> normalization/layernorm_residual_add -> attention/gated_mlp_block -> branch/residual_add_relu")
