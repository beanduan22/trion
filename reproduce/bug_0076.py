#!/usr/bin/env python3
"""
bug_0076 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.2887
Input shape    : [1, 3, 32, 32]
Patterns       : layout/transpose_conv_nhwc -> fusion/conv_relu_conv_bn -> normalization/layernorm_residual_add -> branch/spatial_attention_cbam -> normalization/manual_group_norm -> fusion/conv_gelu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.6490'}

Usage:
    python reproduce/bug_0076.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0076.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.6490)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.6490)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.6490)")


if __name__ == "__main__":
    print("Reproducing bug_0076 [DISCREPANCY]")
    print("  patterns : layout/transpose_conv_nhwc -> fusion/conv_relu_conv_bn -> normalization/layernorm_residual_add -> branch/spatial_attention_cbam -> normalization/manual_group_norm -> fusion/conv_gelu")
