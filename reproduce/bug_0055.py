#!/usr/bin/env python3
"""
bug_0055 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.5169
Input shape    : [1, 256]
Patterns       : normalization/instance_norm_1d -> fusion/matmul_bias_gelu -> fusion/matmul_bias_gelu -> fusion/linear_layernorm_gelu -> normalization/ada_layer_norm -> constant/cast_roundtrip

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2584'}

Usage:
    python reproduce/bug_0055.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0055.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2584)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2584)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2584)")


if __name__ == "__main__":
    print("Reproducing bug_0055 [DISCREPANCY]")
    print("  patterns : normalization/instance_norm_1d -> fusion/matmul_bias_gelu -> fusion/matmul_bias_gelu -> fusion/linear_layernorm_gelu -> normalization/ada_layer_norm -> constant/cast_roundtrip")
    print("  input    : [1, 256]")
