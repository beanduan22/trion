#!/usr/bin/env python3
"""
bug_0115 — DISCREPANCY bug
============================================================
Compiler class : onnxruntime
Affected       : onnxruntime
Total score    : 0.2991
Input shape    : [1, 3, 32, 32]
Patterns       : constant/pow_canonical -> branch/multi_scale_conv_branch -> constant/transpose_inverse_cancel -> broadcast/floor_ceil_round -> fusion/conv_bn_relu_maxpool -> layout/reshape_transpose_reshape

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'onnxruntime': '0.1391'}

Usage:
    python reproduce/bug_0115.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0115.onnx")

def test_discrepancy_onnxruntime():
    """
    DISCREPANCY bug on onnxruntime: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1391)
    """
    ...
    if rd > 0.05:
        print(f"  [onnxruntime] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1391)")
    else:
        print(f"  [onnxruntime] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1391)")


if __name__ == "__main__":
    print("Reproducing bug_0115 [DISCREPANCY]")
    print("  patterns : constant/pow_canonical -> branch/multi_scale_conv_branch -> constant/transpose_inverse_cancel -> broadcast/floor_ceil_round -> fusion/conv_bn_relu_maxpool -> layout/reshape_transpose_reshape")
