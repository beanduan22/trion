#!/usr/bin/env python3
"""
bug_0061 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.7644
Input shape    : [1, 64, 16, 16]
Patterns       : normalization/power_norm -> constant/pow_canonical -> fusion/conv_bn_hardswish -> layout/ceil_mode_avg_pool_conv -> normalization/variance_whitening -> constant/div_by_constant

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.8165'}

Usage:
    python reproduce/bug_0061.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0061.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.8165)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.8165)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.8165)")


if __name__ == "__main__":
    print("Reproducing bug_0061 [DISCREPANCY]")
    print("  patterns : normalization/power_norm -> constant/pow_canonical -> fusion/conv_bn_hardswish -> layout/ceil_mode_avg_pool_conv -> normalization/variance_whitening -> constant/div_by_constant")
