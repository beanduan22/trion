#!/usr/bin/env python3
"""
bug_0083 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.6187
Input shape    : [1, 32, 32, 32]
Patterns       : branch/se_residual -> broadcast/sqrt_div_rms -> branch/aspp_dilated_branch -> constant/self_sub_zero -> branch/dense_residual_block -> constant/where_const_cond

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2794'}

Usage:
    python reproduce/bug_0083.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0083.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2794)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2794)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2794)")


if __name__ == "__main__":
    print("Reproducing bug_0083 [DISCREPANCY]")
    print("  patterns : branch/se_residual -> broadcast/sqrt_div_rms -> branch/aspp_dilated_branch -> constant/self_sub_zero -> branch/dense_residual_block -> constant/where_const_cond")
