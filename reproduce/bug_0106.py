#!/usr/bin/env python3
"""
bug_0106 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1986
Input shape    : [1, 64, 16, 16]
Patterns       : fusion/conv_bn_relu_maxpool -> fusion/conv_bn_elu -> branch/aspp_dilated_branch -> fusion/conv_bn_relu -> normalization/layernorm_residual_add -> fusion/conv_bn_sigmoid

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1286'}

Usage:
    python reproduce/bug_0106.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0106.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1286)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1286)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1286)")


if __name__ == "__main__":
    print("Reproducing bug_0106 [DISCREPANCY]")
    print("  patterns : fusion/conv_bn_relu_maxpool -> fusion/conv_bn_elu -> branch/aspp_dilated_branch -> fusion/conv_bn_relu -> normalization/layernorm_residual_add -> fusion/conv_bn_sigmoid")
