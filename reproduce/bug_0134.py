#!/usr/bin/env python3
"""
bug_0134 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1335
Input shape    : [1, 32, 16, 16]
Patterns       : normalization/stable_softmax -> fusion/conv_transpose_bn_relu -> fusion/pointwise_dw_block -> branch/dense_residual_block -> branch/fpn_branch -> broadcast/swish

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0653'}

Usage:
    python reproduce/bug_0134.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0134.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0653)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0653)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0653)")


if __name__ == "__main__":
    print("Reproducing bug_0134 [DISCREPANCY]")
    print("  patterns : normalization/stable_softmax -> fusion/conv_transpose_bn_relu -> fusion/pointwise_dw_block -> branch/dense_residual_block -> branch/fpn_branch -> broadcast/swish")
