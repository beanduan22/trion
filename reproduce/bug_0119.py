#!/usr/bin/env python3
"""
bug_0119 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.2886
Input shape    : [1, 3, 32, 32]
Patterns       : fusion/pointwise_dw_block -> branch/concat_conv -> constant/sqrt_reciprocal_mul -> fusion/dilated_conv_bn_relu -> constant/self_sub_zero -> fusion/conv_asym_pad_bn

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0630'}

Usage:
    python reproduce/bug_0119.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0119.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0630)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0630)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0630)")


if __name__ == "__main__":
    print("Reproducing bug_0119 [DISCREPANCY]")
    print("  patterns : fusion/pointwise_dw_block -> branch/concat_conv -> constant/sqrt_reciprocal_mul -> fusion/dilated_conv_bn_relu -> constant/self_sub_zero -> fusion/conv_asym_pad_bn")
