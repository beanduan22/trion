#!/usr/bin/env python3
"""
bug_0102 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1204
Input shape    : [1, 3, 32, 32]
Patterns       : broadcast/mul_add_relu -> constant/constant_add_mul -> layout/ceil_mode_avg_pool_conv -> broadcast/cumsum -> fusion/conv_bn_relu6 -> fusion/dilated_conv_bn_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0585'}

Usage:
    python reproduce/bug_0102.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0102.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0585)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0585)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0585)")


if __name__ == "__main__":
    print("Reproducing bug_0102 [DISCREPANCY]")
    print("  patterns : broadcast/mul_add_relu -> constant/constant_add_mul -> layout/ceil_mode_avg_pool_conv -> broadcast/cumsum -> fusion/conv_bn_relu6 -> fusion/dilated_conv_bn_relu")
