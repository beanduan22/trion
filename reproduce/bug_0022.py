#!/usr/bin/env python3
"""
bug_0022 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : openvino, tflite, torch_compile
Total score    : 4.8872
Input shape    : [1, 64, 16, 16]
Patterns       : broadcast/hard_clamp_norm -> branch/aspp_dilated_branch -> branch/concat_conv -> branch/channel_gating_branch -> branch/spatial_attention_cbam -> fusion/conv_add_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.4062', 'tflite': '1.0000', 'openvino': '1.0000'}

Usage:
    python reproduce/bug_0022.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0022.onnx")
...
def test_discrepancy_torch_compile():
    ...
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.4062)
    """
    import onnx, numpy as np
    model  = onnx.load(ONNX_PATH)
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 64, 16, 16]).astype(np.float32)}
    ...
    rd = float(np.linalg.norm(a[:n] - b[:n])) / max(float(np.linalg.norm(b[:n])), 1e-8)
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.4062)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.4062)")


if __name__ == "__main__":
    print("Reproducing bug_0022 [DISCREPANCY]")
    print("  patterns : broadcast/hard_clamp_norm -> branch/aspp_dilated_branch -> branch/concat_conv -> branch/channel_gating_branch -> branch/spatial_attention_cbam -> fusion/conv_add_relu")
