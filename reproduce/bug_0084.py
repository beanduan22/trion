#!/usr/bin/env python3
"""
bug_0084 — DISCREPANCY bug
============================================================
Compiler class : onnxruntime
Affected       : onnxruntime, torch_compile
Total score    : 1.7688
Input shape    : [1, 3, 32, 32]
Patterns       : layout/reflect_pad_conv -> fusion/dilated_conv_bn_relu -> broadcast/clipped_affine -> normalization/layernorm_residual_add -> fusion/conv_bn_relu6 -> normalization/instancenorm_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'onnxruntime': '0.1963', 'torch_compile': '0.0643'}

Usage:
    python reproduce/bug_0084.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0084.onnx")

def test_discrepancy_onnxruntime():
    """
    DISCREPANCY bug on onnxruntime: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1963)
    """
    ...
    if rd > 0.05:
        print(f"  [onnxruntime] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1963)")
    else:
        print(f"  [onnxruntime] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1963)")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0643)
    """
    import onnx, numpy as np
    model  = onnx.load(ONNX_PATH)
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 3, 32, 32]).astype(np.float32)}
    ...
    rd = float(np.linalg.norm(a[:n] - b[:n])) / max(float(np.linalg.norm(b[:n])), 1e-8)
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0643)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0643)")


if __name__ == "__main__":
    print("Reproducing bug_0084 [DISCREPANCY]")
    print("  patterns : layout/reflect_pad_conv -> fusion/dilated_conv_bn_relu -> broadcast/clipped_affine -> normalization/layernorm_residual_add -> fusion/conv_bn_relu6 -> normalization/instancenorm_relu")
