#!/usr/bin/env python3
"""
bug_0077 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.7157
Input shape    : [1, 128, 32, 32]
Patterns       : layout/pad_conv -> layout/transpose_conv_nhwc -> branch/concat_conv -> layout/space_to_depth_block -> fusion/grouped_conv_bn_relu -> constant/reshape_reshape_cancel

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1465'}

Usage:
    python reproduce/bug_0077.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0077.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1465)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1465)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1465)")


if __name__ == "__main__":
    print("Reproducing bug_0077 [DISCREPANCY]")
    print("  patterns : layout/pad_conv -> layout/transpose_conv_nhwc -> branch/concat_conv -> layout/space_to_depth_block -> fusion/grouped_conv_bn_relu -> constant/reshape_reshape_cancel")
