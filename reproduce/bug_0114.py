#!/usr/bin/env python3
"""
bug_0114 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.3645
Input shape    : [1, 32, 16, 16]
Patterns       : constant/cast_roundtrip -> branch/spatial_attention_cbam -> normalization/batchnorm_relu6 -> normalization/rmsnorm -> layout/reshape_softmax_reshape -> fusion/conv_bn_relu_maxpool

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0905'}

Usage:
    python reproduce/bug_0114.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0114.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0905)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0905)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0905)")


if __name__ == "__main__":
    print("Reproducing bug_0114 [DISCREPANCY]")
    print("  patterns : constant/cast_roundtrip -> branch/spatial_attention_cbam -> normalization/batchnorm_relu6 -> normalization/rmsnorm -> layout/reshape_softmax_reshape -> fusion/conv_bn_relu_maxpool")
