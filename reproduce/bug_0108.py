#!/usr/bin/env python3
"""
bug_0108 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1281
Input shape    : [1, 32, 16, 16]
Patterns       : fusion/conv_transpose_bn_relu -> normalization/manual_group_norm -> layout/pad_conv -> branch/multi_scale_conv_branch -> normalization/manual_layernorm -> fusion/conv_bn_silu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0623'}

Usage:
    python reproduce/bug_0108.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0108.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0623)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0623)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0623)")


if __name__ == "__main__":
    print("Reproducing bug_0108 [DISCREPANCY]")
    print("  patterns : fusion/conv_transpose_bn_relu -> normalization/manual_group_norm -> layout/pad_conv -> branch/multi_scale_conv_branch -> normalization/manual_layernorm -> fusion/conv_bn_silu")
