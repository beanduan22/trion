#!/usr/bin/env python3
"""
bug_0023 — CRASH+DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : openvino, tensorflow, tflite, tvm, xla
Total score    : 6.0021
Input shape    : [1, 3, 32, 32]
Patterns       : layout/dilated_max_pool -> fusion/conv_bn_sigmoid -> broadcast/log_clamp -> constant/identity_chain -> broadcast/sqrt_div_rms -> broadcast/mul_add_relu

Bug type: CRASH+DISCREPANCY
Crash errors:
  [xla+opt]: add got incompatible shapes for broadcasting: (1, 64, 32, 32), (1, 64, 30, 30).
  [xla-opt]: add got incompatible shapes for broadcasting: (1, 64, 32, 32), (1, 64, 30, 30).
  [tvm+opt]: add got incompatible shapes for broadcasting: (1, 64, 32, 32), (1, 64, 30, 30).
  [tvm-opt]: add got incompatible shapes for broadcasting: (1, 64, 32, 32), (1, 64, 30, 30).
...
Usage:
    python reproduce/bug_0023.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0023.onnx")
...
def test_crash_tvm():
    """
    CRASH bug on tvm.
    Expected error: 'add got incompatible shapes for broadcasting: (1, 64, 32, 32), (1, 64, 30, 30).'
    """
    import onnx, numpy as np
    model  = onnx.load(ONNX_PATH)
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 3, 32, 32]).astype(np.float32)}
...
if __name__ == "__main__":
    print("Reproducing bug_0023 [CRASH+DISCREPANCY]")
    print("  patterns : layout/dilated_max_pool -> fusion/conv_bn_sigmoid -> broadcast/log_clamp -> constant/identity_chain -> broadcast/sqrt_div_rms -> broadcast/mul_add_relu")
