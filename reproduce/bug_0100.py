#!/usr/bin/env python3
"""
bug_0100 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1814
Input shape    : [1, 64, 32, 32]
Patterns       : branch/fpn_branch -> broadcast/cumsum -> branch/multi_scale_conv_branch -> normalization/groupnorm_relu -> branch/max_avg_pool_fusion -> normalization/manual_layernorm

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0567'}

Usage:
    python reproduce/bug_0100.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0100.onnx")
...
def test_discrepancy_torch_compile():
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0567)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0567)")


if __name__ == "__main__":
    print("Reproducing bug_0100 [DISCREPANCY]")
    print("  patterns : branch/fpn_branch -> broadcast/cumsum -> branch/multi_scale_conv_branch -> normalization/groupnorm_relu -> branch/max_avg_pool_fusion -> normalization/manual_layernorm")
