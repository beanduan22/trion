#!/usr/bin/env python3
"""
bug_0006 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : openvino, tflite
Total score    : 3.3215
Input shape    : [1, 64, 16, 16]
Patterns       : broadcast/l2_norm -> fusion/conv_bn_leakyrelu -> normalization/batchnorm_eval -> broadcast/cumsum -> constant/transpose_inverse_cancel -> constant/div_by_constant

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '1.0000', 'openvino': '0.6294'}

Usage:
    python reproduce/bug_0006.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0006.onnx")

def test_discrepancy_openvino():
    """
    DISCREPANCY bug on openvino: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.6294)
    """
    ...
    if rd > 0.05:
        print(f"  [openvino] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.6294)")
    else:
        print(f"  [openvino] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.6294)")
...
if __name__ == "__main__":
    print("Reproducing bug_0006 [DISCREPANCY]")
    print("  patterns : broadcast/l2_norm -> fusion/conv_bn_leakyrelu -> normalization/batchnorm_eval -> broadcast/cumsum -> constant/transpose_inverse_cancel -> constant/div_by_constant")
