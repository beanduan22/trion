#!/usr/bin/env python3
"""
bug_0080 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.7230
Input shape    : [1, 32, 32, 32]
Patterns       : fusion/conv_bn_relu_maxpool -> broadcast/sub_mul_add -> fusion/conv_add_relu -> layout/reflect_pad_conv -> fusion/conv_bn_elu -> broadcast/euclidean_norm_broadcast

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0594'}

Usage:
    python reproduce/bug_0080.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0080.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0594)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0594)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0594)")


if __name__ == "__main__":
    print("Reproducing bug_0080 [DISCREPANCY]")
    print("  patterns : fusion/conv_bn_relu_maxpool -> broadcast/sub_mul_add -> fusion/conv_add_relu -> layout/reflect_pad_conv -> fusion/conv_bn_elu -> broadcast/euclidean_norm_broadcast")
