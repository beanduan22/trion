#!/usr/bin/env python3
"""
bug_0053 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1293
Input shape    : [1, 32, 16, 16]
Patterns       : constant/cast_roundtrip -> layout/reshape_layernorm_reshape -> fusion/conv_add_relu -> layout/reshape_softmax_reshape -> constant/log_exp_cancel -> broadcast/log_clamp

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0627'}

Usage:
    python reproduce/bug_0053.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0053.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0627)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0627)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0627)")


if __name__ == "__main__":
    print("Reproducing bug_0053 [DISCREPANCY]")
    print("  patterns : constant/cast_roundtrip -> layout/reshape_layernorm_reshape -> fusion/conv_add_relu -> layout/reshape_softmax_reshape -> constant/log_exp_cancel -> broadcast/log_clamp")
