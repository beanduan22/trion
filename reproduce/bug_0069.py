#!/usr/bin/env python3
"""
bug_0069 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1118
Input shape    : [1, 3, 32, 32]
Patterns       : layout/slice_pad_concat -> constant/slice_full_range -> branch/residual_add_relu -> constant/self_sub_zero -> constant/cast_roundtrip -> fusion/conv_bn_sigmoid

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0558'}

Usage:
    python reproduce/bug_0069.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0069.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0558)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0558)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0558)")


if __name__ == "__main__":
    print("Reproducing bug_0069 [DISCREPANCY]")
    print("  patterns : layout/slice_pad_concat -> constant/slice_full_range -> branch/residual_add_relu -> constant/self_sub_zero -> constant/cast_roundtrip -> fusion/conv_bn_sigmoid")
