#!/usr/bin/env python3
"""
bug_0170 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : tflite
Total score    : 0.1116
Input shape    : [1, 64, 256]
Patterns       : attention/gated_mlp_block -> constant/sqrt_reciprocal_mul -> layout/squeeze_unsqueeze -> layout/squeeze_unsqueeze -> broadcast/sqrt_div_rms -> branch/residual_add_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '0.1116'}

Usage:
    python reproduce/bug_0170.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0170.onnx")

def test_discrepancy_tflite():
    """
    DISCREPANCY bug on tflite: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1116)
    """
    ...
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1116)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1116)")


if __name__ == "__main__":
    print("Reproducing bug_0170 [DISCREPANCY]")
    print("  patterns : attention/gated_mlp_block -> constant/sqrt_reciprocal_mul -> layout/squeeze_unsqueeze -> layout/squeeze_unsqueeze -> broadcast/sqrt_div_rms -> branch/residual_add_relu")
