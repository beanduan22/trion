#!/usr/bin/env python3
"""
bug_0123 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.4928
Input shape    : [1, 256]
Patterns       : fusion/matmul_bias_sigmoid -> constant/cast_roundtrip -> constant/div_by_constant -> normalization/layernorm_dropout_identity -> normalization/power_norm -> constant/constant_add_mul

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1232'}

Usage:
    python reproduce/bug_0123.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0123.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1232)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1232)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1232)")


if __name__ == "__main__":
    print("Reproducing bug_0123 [DISCREPANCY]")
    print("  patterns : fusion/matmul_bias_sigmoid -> constant/cast_roundtrip -> constant/div_by_constant -> normalization/layernorm_dropout_identity -> normalization/power_norm -> constant/constant_add_mul")
    print("  input    : [1, 256]")
