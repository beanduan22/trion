#!/usr/bin/env python3
"""
bug_0016 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : openvino, tflite, torch_compile
Total score    : 5.2978
Input shape    : [1, 32, 256]
Patterns       : layout/squeeze_unsqueeze -> attention/causal_masked_attention -> attention/scaled_dot_product_attention -> constant/cast_roundtrip -> normalization/layernorm_relu -> branch/add_layernorm

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2505', 'tflite': '1.0000', 'openvino': '1.0000'}

Usage:
    python reproduce/bug_0016.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0016.onnx")
...
def test_discrepancy_openvino():
    ...
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 32, 256]).astype(np.float32)}
    try:
...
def test_discrepancy_torch_compile():
    ...
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2505)
    """
    import onnx, numpy as np
    model  = onnx.load(ONNX_PATH)
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 32, 256]).astype(np.float32)}
    ...
    rd = float(np.linalg.norm(a[:n] - b[:n])) / max(float(np.linalg.norm(b[:n])), 1e-8)
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2505)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2505)")


if __name__ == "__main__":
    print("Reproducing bug_0016 [DISCREPANCY]")
    print("  patterns : layout/squeeze_unsqueeze -> attention/causal_masked_attention -> attention/scaled_dot_product_attention -> constant/cast_roundtrip -> normalization/layernorm_relu -> branch/add_layernorm")
    print("  input    : [1, 32, 256]")
