#!/usr/bin/env python3
"""
bug_0168 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : tflite
Total score    : 0.1965
Input shape    : [1, 64, 256]
Patterns       : attention/transformer_ffn -> attention/kv_cache_attention -> layout/squeeze_unsqueeze -> constant/sqrt_reciprocal_mul -> broadcast/euclidean_norm_broadcast -> normalization/rmsnorm

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '0.1965'}

Usage:
    python reproduce/bug_0168.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0168.onnx")

def test_discrepancy_tflite():
    """
    DISCREPANCY bug on tflite: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1965)
    """
    ...
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1965)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1965)")


if __name__ == "__main__":
    print("Reproducing bug_0168 [DISCREPANCY]")
    print("  patterns : attention/transformer_ffn -> attention/kv_cache_attention -> layout/squeeze_unsqueeze -> constant/sqrt_reciprocal_mul -> broadcast/euclidean_norm_broadcast -> normalization/rmsnorm")
