#!/usr/bin/env python3
"""
bug_0038 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.0580
Input shape    : [1, 3, 32, 32]
Patterns       : normalization/layernorm_residual_add -> broadcast/hard_clamp_norm -> normalization/layernorm_relu -> branch/add_layernorm -> fusion/pointwise_dw_block -> normalization/batchnorm_eval

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0576'}

Usage:
    python reproduce/bug_0038.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0038.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0576)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0576)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0576)")


if __name__ == "__main__":
    print("Reproducing bug_0038 [DISCREPANCY]")
    print("  patterns : normalization/layernorm_residual_add -> broadcast/hard_clamp_norm -> normalization/layernorm_relu -> branch/add_layernorm -> fusion/pointwise_dw_block -> normalization/batchnorm_eval")
