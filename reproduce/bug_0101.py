#!/usr/bin/env python3
"""
bug_0101 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.2143
Input shape    : [1, 3, 32, 32]
Patterns       : layout/transpose_conv_nhwc -> fusion/conv_bn_elu -> constant/cast_roundtrip -> fusion/grouped_conv_bn_relu -> normalization/ada_layer_norm -> normalization/layernorm_temperature

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0524'}

Usage:
    python reproduce/bug_0101.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0101.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0524)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0524)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0524)")


if __name__ == "__main__":
    print("Reproducing bug_0101 [DISCREPANCY]")
    print("  patterns : layout/transpose_conv_nhwc -> fusion/conv_bn_elu -> constant/cast_roundtrip -> fusion/grouped_conv_bn_relu -> normalization/ada_layer_norm -> normalization/layernorm_temperature")
