#!/usr/bin/env python3
"""
bug_0010 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : openvino, tflite, torch_compile
Total score    : 4.5520
Input shape    : [1, 64, 16, 16]
Patterns       : normalization/layernorm_temperature -> fusion/conv_relu_conv_bn -> fusion/conv_bn_relu6 -> normalization/stable_softmax -> layout/transpose_conv_nhwc -> constant/learned_temperature_scale

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0931', 'tflite': '1.0000', 'openvino': '1.0000'}

Usage:
    python reproduce/bug_0010.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0010.onnx")
...
def test_discrepancy_torch_compile():
    ...
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0931)
    """
    import onnx, numpy as np
    model  = onnx.load(ONNX_PATH)
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 64, 16, 16]).astype(np.float32)}
    ...
    rd = float(np.linalg.norm(a[:n] - b[:n])) / max(float(np.linalg.norm(b[:n])), 1e-8)
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0931)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0931)")


if __name__ == "__main__":
    print("Reproducing bug_0010 [DISCREPANCY]")
    print("  patterns : normalization/layernorm_temperature -> fusion/conv_relu_conv_bn -> fusion/conv_bn_relu6 -> normalization/stable_softmax -> layout/transpose_conv_nhwc -> constant/learned_temperature_scale")
