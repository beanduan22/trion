#!/usr/bin/env python3
"""
bug_0089 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.4324
Input shape    : [1, 128, 16, 16]
Patterns       : broadcast/log_clamp -> branch/aspp_dilated_branch -> constant/constant_add_mul -> branch/aspp_dilated_branch -> fusion/pointwise_dw_block -> constant/cast_roundtrip

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.6740'}

Usage:
    python reproduce/bug_0089.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0089.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.6740)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.6740)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.6740)")


if __name__ == "__main__":
    print("Reproducing bug_0089 [DISCREPANCY]")
    print("  patterns : broadcast/log_clamp -> branch/aspp_dilated_branch -> constant/constant_add_mul -> branch/aspp_dilated_branch -> fusion/pointwise_dw_block -> constant/cast_roundtrip")
