#!/usr/bin/env python3
"""
bug_0063 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.4173
Input shape    : [1, 3, 32, 32]
Patterns       : broadcast/floor_ceil_round -> layout/dilated_max_pool -> fusion/depthwise_conv_bn_relu -> fusion/conv_bn_relu6 -> normalization/rmsnorm -> branch/aspp_dilated_branch

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1654'}

Usage:
    python reproduce/bug_0063.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0063.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1654)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1654)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1654)")


if __name__ == "__main__":
    print("Reproducing bug_0063 [DISCREPANCY]")
    print("  patterns : broadcast/floor_ceil_round -> layout/dilated_max_pool -> fusion/depthwise_conv_bn_relu -> fusion/conv_bn_relu6 -> normalization/rmsnorm -> branch/aspp_dilated_branch")
