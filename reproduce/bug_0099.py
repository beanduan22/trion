#!/usr/bin/env python3
"""
bug_0099 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.1496
Input shape    : [1, 128, 16, 16]
Patterns       : branch/se_block -> layout/ceil_mode_avg_pool_conv -> fusion/dilated_conv_bn_relu -> broadcast/l2_norm -> broadcast/l2_norm -> fusion/conv_add_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0757'}

Usage:
    python reproduce/bug_0099.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0099.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0757)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0757)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0757)")


if __name__ == "__main__":
    print("Reproducing bug_0099 [DISCREPANCY]")
    print("  patterns : branch/se_block -> layout/ceil_mode_avg_pool_conv -> fusion/dilated_conv_bn_relu -> broadcast/l2_norm -> broadcast/l2_norm -> fusion/conv_add_relu")
