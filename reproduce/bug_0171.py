#!/usr/bin/env python3
"""
bug_0171 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : tflite
Total score    : 0.0951
Input shape    : [1, 3, 32, 32]
Patterns       : broadcast/expand_add_mul -> constant/log_exp_cancel -> fusion/conv_bn_relu -> branch/residual_add_relu -> normalization/variance_whitening -> constant/sqrt_reciprocal_mul

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '0.0951'}

Usage:
    python reproduce/bug_0171.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0171.onnx")

def test_discrepancy_tflite():
    """
    DISCREPANCY bug on tflite: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0951)
    """
    ...
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0951)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0951)")


if __name__ == "__main__":
    print("Reproducing bug_0171 [DISCREPANCY]")
    print("  patterns : broadcast/expand_add_mul -> constant/log_exp_cancel -> fusion/conv_bn_relu -> branch/residual_add_relu -> normalization/variance_whitening -> constant/sqrt_reciprocal_mul")
