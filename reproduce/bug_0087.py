#!/usr/bin/env python3
"""
bug_0087 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.3504
Input shape    : [1, 256]
Patterns       : normalization/stable_softmax -> broadcast/sub_mul_add -> broadcast/where_mask -> constant/mul_by_reciprocal -> constant/cast_roundtrip -> branch/residual_add_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1752'}

Usage:
    python reproduce/bug_0087.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0087.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1752)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1752)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1752)")


if __name__ == "__main__":
    print("Reproducing bug_0087 [DISCREPANCY]")
    print("  patterns : normalization/stable_softmax -> broadcast/sub_mul_add -> broadcast/where_mask -> constant/mul_by_reciprocal -> constant/cast_roundtrip -> branch/residual_add_relu")
    print("  input    : [1, 256]")
