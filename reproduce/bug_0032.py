...
Patterns       : constant/cast_roundtrip -> broadcast/abs_neg_relu -> layout/pad_conv -> layout/ceil_mode_avg_pool_conv -> normalization/layernorm_residual_add -> fusion/conv_bn_relu6

Bug type: CRASH
Crash errors:
  [pytorch_eager]: [ONNXRuntimeError] : 1 : FAIL : Node () Op (Add) [ShapeInferenceError] Incompatible dimensions
  [onnxruntime+opt]: [ONNXRuntimeError] : 1 : FAIL : Node () Op (Add) [ShapeInferenceError] Incompatible dimensions
  [onnxruntime-opt]: [ONNXRuntimeError] : 1 : FAIL : Node () Op (Add) [ShapeInferenceError] Incompatible dimensions
  [torchscript+opt]: 'n500_cbrelu6_cv'
  [torchscript-opt]: 'n500_cbrelu6_cv'
  [torch_compile+opt]: 'n500_cbrelu6_cv'
  [torch_compile-opt]: 'n500_cbrelu6_cv'
...
[ONNX Frontend] Conversion failed for Conv-17
While v
  [openvino-opt]: Check 'false' failed at src/frontends/onnx/frontend/src/frontend.cpp:387:
FrontEnd API failed with GeneralFailure:
Errors during ONNX translation:
...
Usage:
    python reproduce/bug_0032.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0032.onnx")

def test_crash_onnxruntime():
    """
    CRASH bug on onnxruntime.
    Expected error: '[ONNXRuntimeError] : 1 : FAIL : Node () Op (Add) [ShapeInferenceError] Incompatible dimensions'
    """
...
def test_crash_torch_compile():
    ...
    Expected error: "'n500_cbrelu6_cv'"
    """
    import onnx, numpy as np
    model  = onnx.load(ONNX_PATH)
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 3, 32, 32]).astype(np.float32)}
...
if __name__ == "__main__":
    print("Reproducing bug_0032 [CRASH]")
    print("  patterns : constant/cast_roundtrip -> broadcast/abs_neg_relu -> layout/pad_conv -> layout/ceil_mode_avg_pool_conv -> normalization/layernorm_residual_add -> fusion/conv_bn_relu6")
