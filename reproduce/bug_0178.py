#!/usr/bin/env python3
"""
bug_0178 — DISCREPANCY bug
============================================================
Compiler class : tensorflow
Affected       : tensorflow
Total score    : 0.2179
Input shape    : [1, 3, 32, 32]
Patterns       : constant/div_by_constant -> broadcast/floor_ceil_round -> broadcast/clipped_affine -> constant/div_by_constant -> layout/reflect_pad_conv -> branch/split_transform_concat

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tensorflow': '0.2179'}

Usage:
    python reproduce/bug_0178.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0178.onnx")

def test_discrepancy_tensorflow():
    """
    DISCREPANCY bug on tensorflow: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2179)
    """
    ...
    if rd > 0.05:
        print(f"  [tensorflow] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2179)")
    else:
        print(f"  [tensorflow] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2179)")


if __name__ == "__main__":
    print("Reproducing bug_0178 [DISCREPANCY]")
    print("  patterns : constant/div_by_constant -> broadcast/floor_ceil_round -> broadcast/clipped_affine -> constant/div_by_constant -> layout/reflect_pad_conv -> branch/split_transform_concat")
