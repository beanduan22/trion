#!/usr/bin/env python3
"""
bug_0121 — DISCREPANCY bug
============================================================
Compiler class : xla
Affected       : onnxruntime, tvm, xla
Total score    : 2.2698
Input shape    : [1, 3, 32, 32]
Patterns       : layout/reflect_pad_conv -> fusion/conv_add_relu -> branch/channel_gating_branch -> normalization/batchnorm_relu6 -> constant/redundant_reshape -> broadcast/floor_ceil_round

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'onnxruntime': '0.2116', 'xla': '0.2116', 'tvm': '0.2116'}

Usage:
    python reproduce/bug_0121.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0121.onnx")

def test_discrepancy_onnxruntime():
    """
    DISCREPANCY bug on onnxruntime: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2116)
    """
    ...
    if rd > 0.05:
        print(f"  [onnxruntime] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2116)")
    else:
        print(f"  [onnxruntime] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2116)")
...
def test_discrepancy_tvm():
    ...
    rd = float(np.linalg.norm(a[:n] - b[:n])) / max(float(np.linalg.norm(b[:n])), 1e-8)
    if rd > 0.05:
        print(f"  [tvm] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2116)")
    else:
        print(f"  [tvm] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2116)")
...
def test_discrepancy_xla():
    ...
    rd = float(np.linalg.norm(a[:n] - b[:n])) / max(float(np.linalg.norm(b[:n])), 1e-8)
    if rd > 0.05:
        print(f"  [xla] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2116)")
    else:
        print(f"  [xla] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2116)")


if __name__ == "__main__":
    print("Reproducing bug_0121 [DISCREPANCY]")
    print("  patterns : layout/reflect_pad_conv -> fusion/conv_add_relu -> branch/channel_gating_branch -> normalization/batchnorm_relu6 -> constant/redundant_reshape -> broadcast/floor_ceil_round")
