#!/usr/bin/env python3
"""
bug_0112 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.0789
Input shape    : [1, 32, 16, 16]
Patterns       : broadcast/sub_mul_add -> layout/unsqueeze_expand_mul -> branch/conv_branch_add -> broadcast/log_clamp -> branch/concat_conv -> constant/self_sub_zero

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0698'}

Usage:
    python reproduce/bug_0112.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0112.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0698)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0698)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0698)")


if __name__ == "__main__":
    print("Reproducing bug_0112 [DISCREPANCY]")
    print("  patterns : broadcast/sub_mul_add -> layout/unsqueeze_expand_mul -> branch/conv_branch_add -> broadcast/log_clamp -> branch/concat_conv -> constant/self_sub_zero")
