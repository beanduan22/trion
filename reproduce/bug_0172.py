#!/usr/bin/env python3
"""
bug_0172 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : tflite
Total score    : 0.1153
Input shape    : [1, 64, 16, 16]
Patterns       : constant/cast_roundtrip -> branch/multi_scale_conv_branch -> broadcast/floor_ceil_round -> layout/slice_pad_concat -> constant/cast_roundtrip -> layout/squeeze_unsqueeze

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '0.1153'}

Usage:
    python reproduce/bug_0172.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0172.onnx")

def test_discrepancy_tflite():
    """
    DISCREPANCY bug on tflite: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1153)
    """
    ...
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1153)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1153)")


if __name__ == "__main__":
    print("Reproducing bug_0172 [DISCREPANCY]")
    print("  patterns : constant/cast_roundtrip -> branch/multi_scale_conv_branch -> broadcast/floor_ceil_round -> layout/slice_pad_concat -> constant/cast_roundtrip -> layout/squeeze_unsqueeze")
