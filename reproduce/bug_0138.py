#!/usr/bin/env python3
"""
bug_0138 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.8527
Input shape    : [1, 16, 256]
Patterns       : broadcast/euclidean_norm_broadcast -> normalization/power_norm -> attention/rotary_embedding_attention -> constant/cast_roundtrip -> broadcast/mul_add_relu -> layout/squeeze_unsqueeze

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2060'}

Usage:
    python reproduce/bug_0138.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0138.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2060)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2060)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2060)")


if __name__ == "__main__":
    print("Reproducing bug_0138 [DISCREPANCY]")
    print("  patterns : broadcast/euclidean_norm_broadcast -> normalization/power_norm -> attention/rotary_embedding_attention -> constant/cast_roundtrip -> broadcast/mul_add_relu -> layout/squeeze_unsqueeze")
