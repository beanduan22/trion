#!/usr/bin/env python3
"""
bug_0091 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.5294
Input shape    : [1, 64, 16, 16]
Patterns       : broadcast/sub_mul_add -> fusion/conv_bn_elu -> constant/self_sub_zero -> fusion/conv_bn_relu -> layout/transpose_conv_nhwc -> normalization/variance_whitening

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.5107'}

Usage:
    python reproduce/bug_0091.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0091.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.5107)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.5107)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.5107)")


if __name__ == "__main__":
    print("Reproducing bug_0091 [DISCREPANCY]")
    print("  patterns : broadcast/sub_mul_add -> fusion/conv_bn_elu -> constant/self_sub_zero -> fusion/conv_bn_relu -> layout/transpose_conv_nhwc -> normalization/variance_whitening")
