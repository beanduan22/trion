#!/usr/bin/env python3
"""
bug_0073 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.4483
Input shape    : [1, 16, 64]
Patterns       : constant/log_exp_cancel -> branch/residual_add_relu -> broadcast/log_clamp -> broadcast/reciprocal_mul -> constant/sqrt_reciprocal_mul -> constant/cast_roundtrip

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2241'}

Usage:
    python reproduce/bug_0073.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0073.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2241)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2241)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2241)")


if __name__ == "__main__":
    print("Reproducing bug_0073 [DISCREPANCY]")
    print("  patterns : constant/log_exp_cancel -> branch/residual_add_relu -> broadcast/log_clamp -> broadcast/reciprocal_mul -> constant/sqrt_reciprocal_mul -> constant/cast_roundtrip")
    print("  input    : [1, 16, 64]")
