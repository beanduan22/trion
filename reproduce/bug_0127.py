#!/usr/bin/env python3
"""
bug_0127 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.2595
Input shape    : [1, 128, 32, 32]
Patterns       : fusion/grouped_conv_bn_relu -> branch/dual_pool_add -> constant/cast_roundtrip -> branch/dense_residual_block -> normalization/power_norm -> constant/sqrt_reciprocal_mul

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1294'}

Usage:
    python reproduce/bug_0127.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0127.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1294)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1294)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1294)")


if __name__ == "__main__":
    print("Reproducing bug_0127 [DISCREPANCY]")
    print("  patterns : fusion/grouped_conv_bn_relu -> branch/dual_pool_add -> constant/cast_roundtrip -> branch/dense_residual_block -> normalization/power_norm -> constant/sqrt_reciprocal_mul")
