#!/usr/bin/env python3
"""
bug_0082 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.3546
Input shape    : [1, 32, 16, 16]
Patterns       : fusion/conv_asym_pad_bn -> broadcast/hard_clamp_norm -> branch/multi_scale_conv_branch -> fusion/grouped_conv_bn_relu -> branch/concat_conv -> normalization/manual_layernorm

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.3299'}

Usage:
    python reproduce/bug_0082.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0082.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.3299)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.3299)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.3299)")


if __name__ == "__main__":
    print("Reproducing bug_0082 [DISCREPANCY]")
    print("  patterns : fusion/conv_asym_pad_bn -> broadcast/hard_clamp_norm -> branch/multi_scale_conv_branch -> fusion/grouped_conv_bn_relu -> branch/concat_conv -> normalization/manual_layernorm")
