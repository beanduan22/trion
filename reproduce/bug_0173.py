#!/usr/bin/env python3
"""
bug_0173 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : tflite
Total score    : 0.0505
Input shape    : [1, 16, 256]
Patterns       : branch/add_layernorm -> broadcast/cumsum -> attention/multi_head_self_attention -> layout/squeeze_unsqueeze -> normalization/layernorm_temperature -> attention/attention_with_bias

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '0.0505'}

Usage:
    python reproduce/bug_0173.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0173.onnx")

def test_discrepancy_tflite():
    """
    DISCREPANCY bug on tflite: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0505)
    """
    ...
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0505)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0505)")


if __name__ == "__main__":
    print("Reproducing bug_0173 [DISCREPANCY]")
    print("  patterns : branch/add_layernorm -> broadcast/cumsum -> attention/multi_head_self_attention -> layout/squeeze_unsqueeze -> normalization/layernorm_temperature -> attention/attention_with_bias")
