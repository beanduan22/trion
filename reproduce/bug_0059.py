#!/usr/bin/env python3
"""
bug_0059 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.3392
Input shape    : [1, 32, 32, 32]
Patterns       : constant/transpose_inverse_cancel -> branch/spatial_attention_cbam -> normalization/batchnorm_eval -> layout/pad_conv -> fusion/gap_linear -> normalization/layernorm_residual_add

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1689'}

Usage:
    python reproduce/bug_0059.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0059.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1689)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1689)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1689)")


if __name__ == "__main__":
    print("Reproducing bug_0059 [DISCREPANCY]")
    print("  patterns : constant/transpose_inverse_cancel -> branch/spatial_attention_cbam -> normalization/batchnorm_eval -> layout/pad_conv -> fusion/gap_linear -> normalization/layernorm_residual_add")
