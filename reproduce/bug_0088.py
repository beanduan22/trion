#!/usr/bin/env python3
"""
bug_0088 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.4432
Input shape    : [1, 64, 256]
Patterns       : layout/squeeze_unsqueeze -> attention/kv_cache_attention -> normalization/rmsnorm -> layout/squeeze_unsqueeze -> constant/cast_roundtrip -> broadcast/l2_norm

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2077'}

Usage:
    python reproduce/bug_0088.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0088.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2077)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2077)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2077)")


if __name__ == "__main__":
    print("Reproducing bug_0088 [DISCREPANCY]")
    print("  patterns : layout/squeeze_unsqueeze -> attention/kv_cache_attention -> normalization/rmsnorm -> layout/squeeze_unsqueeze -> constant/cast_roundtrip -> broadcast/l2_norm")
