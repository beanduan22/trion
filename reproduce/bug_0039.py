#!/usr/bin/env python3
"""
bug_0039 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.4290
Input shape    : [1, 64, 256]
Patterns       : normalization/layernorm_temperature -> attention/gated_mlp_block -> constant/where_const_cond -> normalization/manual_layernorm -> constant/cast_roundtrip -> branch/add_layernorm

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.2066'}

Usage:
    python reproduce/bug_0039.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0039.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.2066)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2066)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2066)")


if __name__ == "__main__":
    print("Reproducing bug_0039 [DISCREPANCY]")
    print("  patterns : normalization/layernorm_temperature -> attention/gated_mlp_block -> constant/where_const_cond -> normalization/manual_layernorm -> constant/cast_roundtrip -> branch/add_layernorm")
