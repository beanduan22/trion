#!/usr/bin/env python3
"""
bug_0036 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.8486
Input shape    : [1, 32, 32, 32]
Patterns       : fusion/conv_asym_pad_bn -> constant/transpose_inverse_cancel -> fusion/depthwise_conv_bn_relu -> constant/constant_add_mul -> broadcast/where_mask -> fusion/conv_bn_leakyrelu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.4213'}

Usage:
    python reproduce/bug_0036.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0036.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.4213)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.4213)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.4213)")


if __name__ == "__main__":
    print("Reproducing bug_0036 [DISCREPANCY]")
    print("  patterns : fusion/conv_asym_pad_bn -> constant/transpose_inverse_cancel -> fusion/depthwise_conv_bn_relu -> constant/constant_add_mul -> broadcast/where_mask -> fusion/conv_bn_leakyrelu")
