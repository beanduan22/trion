#!/usr/bin/env python3
"""
bug_0081 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.2052
Input shape    : [1, 128, 16, 16]
Patterns       : fusion/dilated_conv_bn_relu -> constant/sqrt_reciprocal_mul -> constant/pad_slice_noop -> fusion/conv_asym_pad_bn -> normalization/batchnorm_relu6 -> branch/aspp_dilated_branch

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1822'}

Usage:
    python reproduce/bug_0081.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0081.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1822)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1822)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1822)")


if __name__ == "__main__":
    print("Reproducing bug_0081 [DISCREPANCY]")
    print("  patterns : fusion/dilated_conv_bn_relu -> constant/sqrt_reciprocal_mul -> constant/pad_slice_noop -> fusion/conv_asym_pad_bn -> normalization/batchnorm_relu6 -> branch/aspp_dilated_branch")
