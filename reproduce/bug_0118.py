#!/usr/bin/env python3
"""
bug_0118 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 0.2156
Input shape    : [1, 32, 32, 32]
Patterns       : normalization/layernorm_dropout_identity -> broadcast/sub_mul_add -> branch/aspp_dilated_branch -> normalization/spatial_reduce_mean -> normalization/variance_whitening -> branch/aspp_dilated_branch

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.0614'}

Usage:
    python reproduce/bug_0118.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0118.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0614)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0614)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0614)")


if __name__ == "__main__":
    print("Reproducing bug_0118 [DISCREPANCY]")
    print("  patterns : normalization/layernorm_dropout_identity -> broadcast/sub_mul_add -> branch/aspp_dilated_branch -> normalization/spatial_reduce_mean -> normalization/variance_whitening -> branch/aspp_dilated_branch")
