#!/usr/bin/env python3
"""
bug_0004 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : openvino, tflite
Total score    : 1.9555
Input shape    : [1, 64]
Patterns       : constant/slice_full_range -> broadcast/swish -> fusion/matmul_bias_sigmoid -> branch/residual_add_relu -> broadcast/exp_div_softplus -> constant/div_by_constant

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '0.7346', 'openvino': '0.2429'}

Usage:
    python reproduce/bug_0004.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0004.onnx")
...
def test_discrepancy_openvino():
    ...
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 64]).astype(np.float32)}
    try:
    ...
    if rd > 0.05:
        print(f"  [openvino] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2429)")
    else:
        print(f"  [openvino] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2429)")

def test_discrepancy_tflite():
    """
    DISCREPANCY bug on tflite: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.7346)
    """
    import onnx, numpy as np
    model  = onnx.load(ONNX_PATH)
    inputs = {model.graph.input[0].name: np.random.default_rng(42).standard_normal([1, 64]).astype(np.float32)}
    ...
    rd = float(np.linalg.norm(a[:n] - b[:n])) / max(float(np.linalg.norm(b[:n])), 1e-8)
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.7346)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.7346)")


if __name__ == "__main__":
    print("Reproducing bug_0004 [DISCREPANCY]")
    print("  patterns : constant/slice_full_range -> broadcast/swish -> fusion/matmul_bias_sigmoid -> branch/residual_add_relu -> broadcast/exp_div_softplus -> constant/div_by_constant")
    print("  input    : [1, 64]")
