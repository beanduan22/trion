#!/usr/bin/env python3
"""
bug_0188 — DISCREPANCY bug
============================================================
Compiler class : tflite
Affected       : tflite
Total score    : 0.1607
Input shape    : [1, 32, 16, 16]
Patterns       : constant/pow_canonical -> broadcast/swish -> normalization/layernorm_dropout_identity -> fusion/conv_relu_conv_bn -> broadcast/l2_norm -> broadcast/where_mask

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'tflite': '0.1607'}

Usage:
    python reproduce/bug_0188.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0188.onnx")
...
def test_discrepancy_tflite():
    ...
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1607)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1607)")


if __name__ == "__main__":
    print("Reproducing bug_0188 [DISCREPANCY]")
    print("  patterns : constant/pow_canonical -> broadcast/swish -> normalization/layernorm_dropout_identity -> fusion/conv_relu_conv_bn -> broadcast/l2_norm -> broadcast/where_mask")
