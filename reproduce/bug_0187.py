...
Discrepancy (rel_diff opt vs noopt): {'tflite': '0.2179'}

Usage:
    python reproduce/bug_0187.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0187.onnx")
...
def test_discrepancy_tflite():
    ...
    if rd > 0.05:
        print(f"  [tflite] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.2179)")
    else:
        print(f"  [tflite] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.2179)")


if __name__ == "__main__":
    print("Reproducing bug_0187 [DISCREPANCY]")
