#!/usr/bin/env python3
"""
bug_0117 — DISCREPANCY bug
============================================================
Compiler class : torch_compile
Affected       : torch_compile
Total score    : 1.1565
Input shape    : [1, 128, 32, 32]
Patterns       : constant/where_const_cond -> branch/aspp_dilated_branch -> branch/multi_scale_conv_branch -> branch/max_avg_pool_fusion -> constant/sqrt_reciprocal_mul -> normalization/batchnorm_relu6

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'torch_compile': '0.1496'}

Usage:
    python reproduce/bug_0117.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0117.onnx")

def test_discrepancy_torch_compile():
    """
    DISCREPANCY bug on torch_compile: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.1496)
    """
    ...
    if rd > 0.05:
        print(f"  [torch_compile] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.1496)")
    else:
        print(f"  [torch_compile] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.1496)")


if __name__ == "__main__":
    print("Reproducing bug_0117 [DISCREPANCY]")
    print("  patterns : constant/where_const_cond -> branch/aspp_dilated_branch -> branch/multi_scale_conv_branch -> branch/max_avg_pool_fusion -> constant/sqrt_reciprocal_mul -> normalization/batchnorm_relu6")
