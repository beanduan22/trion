#!/usr/bin/env python3
"""
bug_0197 — DISCREPANCY bug
============================================================
Compiler class : openvino
Affected       : openvino
Total score    : 0.0764
Input shape    : [1, 3, 32, 32]
Patterns       : normalization/manual_layernorm -> fusion/conv_bn_relu_maxpool -> broadcast/sub_mul_add -> broadcast/floor_ceil_round -> normalization/groupnorm_relu -> fusion/grouped_conv_bn_relu

Bug type: DISCREPANCY

Discrepancy (rel_diff opt vs noopt): {'openvino': '0.0764'}

Usage:
    python reproduce/bug_0197.py
"""
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

ONNX_PATH = os.path.join(os.path.dirname(__file__), "..", "bugs", "bug_0197.onnx")

def test_discrepancy_openvino():
    """
    DISCREPANCY bug on openvino: optimized output differs from non-optimized.
    Expected rel_diff(opt_output, noopt_output) > 0.05  (stored: 0.0764)
    """
    ...
    if rd > 0.05:
        print(f"  [openvino] DISCREPANCY confirmed  rel_diff={rd:.4f}  (expected>0.05, stored=0.0764)")
    else:
        print(f"  [openvino] no discrepancy  rel_diff={rd:.4f}  (may be fixed, stored=0.0764)")


if __name__ == "__main__":
    print("Reproducing bug_0197 [DISCREPANCY]")
    print("  patterns : normalization/manual_layernorm -> fusion/conv_bn_relu_maxpool -> broadcast/sub_mul_add -> broadcast/floor_ceil_round -> normalization/groupnorm_relu -> fusion/grouped_conv_bn_relu")
